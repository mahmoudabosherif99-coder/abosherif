...
// مفاتيح VAPID الخاصة بمشروع "مزارع أبوشريف" — المفتاح العام موجود في src/App.jsx برضو
...
exports.handler = async function (event) {
  ...
  let subs = [];
  try {
    const res = await fetch(`${SUPA_URL}/rest/v1/push_subscriptions?select=*`, { headers: supaHeaders });
    subs = await res.json();
  ...
  return {
    ...
    body: JSON.stringify({ total: subs.length, sent, failed }),
  };
};
